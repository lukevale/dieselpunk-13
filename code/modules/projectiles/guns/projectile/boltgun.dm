...
/obj/item/weapon/gun/projectile/boltgun/handle_post_fire(mob/user)
	...
	if(bolt_training && user.stats.getPerk(PERK_BOLT_REFLECT) && loaded.len>0)
		...
		bolt_act(user) //bolt_act gets called on the gun already. bolt_act(src) codewise means that it is the gun performing the proc on itself, instead of the user performing it on the gun.
		bolt_act(user)
		return
	if(bolt_training && user.stats.getPerk(PERK_BOLT_REFLECT) && loaded.len==0)
